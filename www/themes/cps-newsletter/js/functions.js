define( [ 'jquery', 'core/theme-app', 'core/theme-tpl-tags', 'core/modules/storage',
		   'theme/js/bootstrap.min', 'theme/js/ct-navbar', 'theme/js/auth/auth-pages', 'theme/js/auth/simple-login',
	...
	var $user_reg = $( '#user-reg' );
	$user_reg.click(function() { window.open(this.href,'targetWindow')
	...
	App.on( 'refresh:end', function( result ) {
		...
		if ( result.ok ) {
			$( '#feedback' ).removeClass( 'error' ).html( 'Most Recent' ).slideDown();
		} else {
	...
	$back_button.click( function ( e ) {
		e.preventDefault();
		App.navigateToPreviousScreen().slideLeft();
	} );
	...
	$( '#container' ).on( 'click', 'li.media', function( e ) {
		...
		App.navigate( navigate_to ).slideRight();;
	} );
	...
	$( "#container" ).on( "click", ".single-content a, .page-content a", function( e ) {
		e.preventDefault();
		openWithInAppBrowser( e.target.href ).slideUp();;
	} );
	 ...
	 */
	 ...
	 $( '#feedback' ).addClass( 'error' ).html( "You seemed offlline :(" ).slideDown();
	 } );
	...
	function openWithInAppBrowser( url ) {
		window.open( url, "_sytem", "location=no" );
	}
